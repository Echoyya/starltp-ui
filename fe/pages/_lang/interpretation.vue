<template>
  <div class="container interpretation">
    <banner :title="$t('menu.interpretation')" :step="step" :banner="banner" :info="$t('order.interpretationInfo')" />
    <div class="form">
      <el-form :model="interpretationForm" :rules="rules" ref="interpretationForm" :inline-message.boolean="true" label-width="250px" class="form-body">
        <el-form-item :label="$t('order.chooseLanguage')" prop="language" :rules="languageRules" :required.boolean="true">
          <el-select v-model="interpretationForm.language.sourceLanguage">
            <el-option v-for="item in config.languages" v-if="item.interpretation" :key="item.value" :label="$t(item.label)" :value="item.value"></el-option>
          </el-select>
          <i class="iconfont">&#xe60c;</i>
          <el-select v-model="interpretationForm.language.targetLanguages" :multiple.boolean="true" collapse-tags>
            <el-option v-for="item in config.languages" v-if="item.interpretation" :key="item.value" :label="$t(item.label)" :value="item.value"></el-option>
          </el-select>
        </el-form-item>
        <el-form-item :label="$t('order.area')" prop="areaOfField" :required.boolean="true">
          <el-select v-model="interpretationForm.areaOfField">
            <el-option v-for="item in config.areaOfField" v-if="item.interpretation" :key="item.value" :label="$t(item.label)" :value="item.value"></el-option>
          </el-select>
        </el-form-item>
        ...
        <el-form-item :label="$t('order.backgroundInfo')">
          <input type="hidden" v-model="interpretationForm.orderFilesList" />
          <star-upload :token="token" :redirect="$i18n.path('interpretation')" :update-files="setUploadFile" />
        </el-form-item>
        <el-form-item :label="$t('order.timeLimit')" prop="time" :rules="timeRules" :required.boolean="true">
          <el-date-picker :picker-options="pickerOptions1" type="datetimerange" v-model="interpretationForm.time" :range-separator="$t('order.to')" :start-placeholder="$t('order.starTime')" :end-placeholder="$t('order.endTime')" @change="timeChange"></el-date-picker>
          <span v-if="timeShow" class="el-upload__tip">{{timeShow}}</span>
        </el-form-item>
        <el-form-item :label="$t('order.claim')" prop="request">
          <el-input type="textarea" v-model="interpretationForm.request" :rows.number="5"></el-input>
        </el-form-item>
        <el-form-item class="agreement" prop="read">
          <el-checkbox v-model="interpretationForm.read" true-label="true" false-label=""></el-checkbox><span class="agreement-title">{{$t('order.agree')}}<nuxt-link :to="$i18n.path('about/agreement')" target="_blank">{{$t('order.protocol')}}</nuxt-link></span>
        </el-form-item>
        <el-form-item>
          <el-button @click="submitForm('interpretationForm')" :loading="loading">{{$t('menu.submit')}}</el-button>
        </el-form-item>
      </el-form>
    </div>
  ...
  methods: {
    ...
    timeChange: function (value) {
      if (value && value.length === 2) {
        let time = value[1].getTime() - value[0].getTime()
        if (time % (60 * 60 * 1000 * 24) === 0) {
          this.timeShow = parseInt(time / (60 * 60 * 1000 * 24)) + this.$t('order.day')
        } else {
          this.timeShow = parseInt(time / (60 * 60 * 1000 * 24)) + this.$t('order.day') + parseInt((time % (60 * 60 * 1000 * 24)) / (60 * 60 * 1000)) + this.$t('order.hour')
        }
      } else {
        this.timeShow = null
      }
