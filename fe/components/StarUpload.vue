<template>
    <div class="star-upload">
        ...
        >
            ...
            <span v-if="fileType" class="el-form-item__error el-form-item__error--inline" @click="(e)=>{e.stopPropagation()}">{{$t('errorCode.fileExt')}}</span>
            <span v-if="fileSize" class="el-form-item__error el-form-item__error--inline" @click="(e)=>{e.stopPropagation()}">{{$t('errorCode.fileSize').replace('*', maxSize / 1048576)}}</span>
            <span v-if="fileLength" class="el-form-item__error el-form-item__error--inline" @click="(e)=>{e.stopPropagation()}">{{$t('errorCode.fileLength')}}</span>
            <div slot="tip" class="el-upload__tip" v-html="tip"></div>
        </el-upload>
        <div style="font-size: 12px; line-height:18px;color: #C0C4CC; margin-top: 5px">{{$t('errorCode.fileTip')}}</div>
